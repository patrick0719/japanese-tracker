...
function SplashScreen() {
  ...
  const logoStyle = {
    enter: { opacity: 0, transform: 'scale(0.82)' },
  ...
  return (
    <div style={{
      ...
      transition: phase === 'exit' ? 'opacity 0.5s ease-in-out 0.15s' : 'none',
      pointerEvents: 'none',
    }}>
      <img
        src={LOGO_DATA_URL}
      ...
      <div style={{
        ...
        {[0, 1, 2].map(i => (
          <div key={i} style={{
            ...
            background: '#8B0000',
            animation: phase === 'hold' ? `dotPulse 1.2s ease-in-out ${i * 0.2}s infinite` : 'none',
          }} />
        ))}
      </div>
      <style>{`
